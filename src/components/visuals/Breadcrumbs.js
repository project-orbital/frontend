...
export default function Breadcrumbs({path, links, title}) {
    ...
    return <Box>
        <Breadcrumb spacing="2px" separator={<RiArrowRightSLine/>}>
            {crumbs.map((crumb, index) => <BreadcrumbItem key={index}>
                <BreadcrumbLink as={Link} to={links[index]} trans>{crumb}</BreadcrumbLink>
            </BreadcrumbItem>)}
        </Breadcrumb>
        <Heading as="h1" size="2xl" pt="10px">{title || crumbs[crumbs.length - 1]}</Heading>
    </Box>;
}
